...
function survey_instances(query, response, postData) {
	...
	authentication.requestflowapi(query, postData, function(flowdata) {
		...
		var body3 =
			...
			'</P>' +
			'</div>' +
			'<button><a href=./sendData?surveyId=' + content.survey_instances[0].surveyId + '> Send data to DHIS2 </a></button>' +
			'</body>' +
			'</html>';
	...
	authentication.requestflowapi(query, postData, function(flowdata) {
		...
		surveyInstanceIdTbl.forEach(function(keyId, index) {
			...
			authentication.requestflowapi(query, postData, function(flowdata) {
				...
				for (var j in questions.question_answers) {
					if (questions.question_answers[j].textualQuestionId !== 'period' && questions.question_answers[j].type !== 'CASCADE') {
						if (questions.question_answers[j].value === 'Yes') {
							questions.question_answers[j].value = true;
						} else if (questions.question_answers[j].value === 'No') {
							questions.question_answers[j].value = false;
						}
			...
			function sendToDHIS2(callback) {
				var cmd = 'curl --data-binary @flowdata.csv "'+ dhis_url +'/api/dataValueSets" -H "Content-Type:application/csv" ' +
